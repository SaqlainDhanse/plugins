...
const ONE_HOUR          = 9000; // 150 hours actually
const ONE_DAY           = 21000;//350 hours actually
...
public plugin_init( ) {
	register_plugin( "Jail: Achievements", "1.0", "Saqlain" );
	...
	ACH_KILL            = RegisterAchievement( "Rebel", "Kill 4 Guards as Prisoner", 4);
	ACH_ASSASIN         = RegisterAchievement( "Assassin", " Kill 25 guards with a knife headshot", 25);
	ACH_ONEHP           = RegisterAchievement( "Lucky", "Win a round with 1 hp", 1);
	ACH_FULLHP          = RegisterAchievement( "Flawless Victory", "Win a round with full hp", 1);
	...
	ACH_DONT_DIE        = RegisterAchievement( "Victory", "Win a round as Guard without any guards dying (atleast 3 Guards)", 1); // FIXME IN MYSQL !! <<500 UGCash<<
	ACH_THREE           = RegisterAchievement( "Three-some", "Kill 3 Guards in a single round with headshot", 1);
	ACH_KID_WITH_GUN    = RegisterAchievement( "Kid with gun", "Kill 25 Guards with a TMP", 25);
	ACH_BLABLA          = RegisterAchievement( "Secret Phrase", "Say secret phrase", 1);
	ACH_GHOST_SNIPER    = RegisterAchievement( "Ghost Sniper", "Kill 10 Guards with a AWP", 10);
	ACH_PLAY_AROUND     = RegisterAchievement( "Play Around", "Spend 150 hours playing on server", 1);
	ACH_DAY_MARATHON    = RegisterAchievement( "Day Marathon", "Spend 350 hours playing on server", 1);
	...
	register_clcmd( "say ugcgaming", "CmdBla" );
	register_clcmd( "set_ach_twomin", "twoMin" );
	register_clcmd( "set_ach_threemin", "threeMin" );
...
public EventRoundEnd( ) {
	...
	{
		iPlayer = iPlayers[i];
		if(get_user_health(iPlayer) == 1)
			AchievementProgress( iPlayer, ACH_ONEHP );	
		if(get_user_health(iPlayer) == 100)
			AchievementProgress( iPlayer, ACH_FULLHP );	
	}

}
/*
public checkTeamDead()
{
...
public EventDeathMsg( ) {
	...
	if( iVictim != iKiller && iTeam == 1 ) {
		...
		if(get_user_team(iKiller) == 1 && get_user_team(iVictim) == 2 && iHeadShot)
			g_iKills[iKiller]++;
